...
const repoBasePath = path.resolve(
...
export function useLogger(prefix: string) {
  ...
  const prefixStr = () => {
    ...
    return `${chalk.cyanBright(
      chalk.bold(`${processedPrefix}:${getCallerLine()}`)
    )}: `;
  };
  return {
    ...console,
          ...
          (message?: any, ...optionalParams: any[]): void => {
            const levelWrapper =
              level === "error"
                ...
                : null;
            return console[
              level as "debug" | "error" | "info" | "log" | "warn" | "trace"
            ](
              prefixStr(),
              chalk.bold(
                levelWrapper ? levelWrapper(`[${level}]`) : `[${level}]`
              ),
              message,
              ...optionalParams
            );
